...
- 选项 **2**：弹出相机选择菜单，再启动 `local_controller.py` + `Camera_multiple_outputs.py`
- 选项 **4**：弹出相机选择菜单，再同时启动 `robot_receiver.py` + `camera_streamer.py`，按 `Ctrl+C` 两者一起停止
- 选项 **5**：弹出相机选择菜单，再进入本地相机测试子菜单
...
- 已设置 → 连接到指定的 OAK-D PoE 地址
- 未设置（选择"两台相机"时）→ depthai 自动检测网络中所有设备
